    ...
    >
      ...
      <div class="text-center mb-8 space-y-2">
        <div class="w-16 h-16 mx-auto rounded-xl bg-gradient-to-br from-zinc-900 to-zinc-700 dark:from-zinc-100 dark:to-zinc-300 flex items-center justify-center animate-float mb-4">
          <UIcon name="i-heroicons-arrow-right-on-rectangle" class="w-8 h-8 text-zinc-100 dark:text-zinc-900" />
        </div>
      ...
      <div class="text-center mt-6 animate-fade-in" style="animation-delay: 0.4s">
        <NuxtLink to="/register" class="text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">
          Don't have an account? <span class="text-zinc-900 dark:text-zinc-100 font-semibold">Sign up</span>
        </NuxtLink>
      </div>
...
async function handleSignIn() {
  ...
  loading.value = false
  if (result.error) {
    toast.add({ title: "Sign in failed", description: result.error.message, color: "error" })
  }
}
